...
STRUCT_DIRS = {
    ...
    "deepseek": STRUCT_ROOT / "deepseek",
    "llama": STRUCT_ROOT / "llama",
    "mistral": STRUCT_ROOT / "mistral",
    "pubchem": STRUCT_ROOT / "pubchem",
}

# Evaluation parameters and settings
MODELS = ["claude", "gemini", "openai", "deepseek", "llama", "mistral"]
...
def classify_evaluation_error(message: str | None, status: str | None = None):
    ...
    if "no json" in lowered and "found" in lowered:
...
def repair_error_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Make error columns consistent before plotting/exporting.

    This keeps old runs useful: if a row was previously saved as code 5
    because Code 2 only saw a FAILED marker, the message is reclassified so
    atom-count, JSON, API/network, etc. appear as their real error types.
    """
    if df.empty:
        return df
    df = df.copy()
    ...
    for idx, row in df.iterrows():
        current_code = row.get("error_code")
        current_label = str(row.get("error_label") or "").strip()
        message = row.get("error_message", "")
        status = row.get("status", "")
        if not str(message or "").strip() and "note" in df.columns:
            note = row.get("note", "")
            marker_details = parse_failure_marker_details(note)
            message = marker_details.get("message", note)

        should_reclassify = (
            pd.isna(current_code)
            or int(current_code) in {5, 8}
            or not current_label
            or current_label.lower() in {"nan", "none"}
        )
        if should_reclassify:
            code, label = classify_evaluation_error(message, status=status)
            df.at[idx, "error_code"] = code
            df.at[idx, "error_label"] = label
        else:
            code_int = int(current_code)
            if not current_label or current_label.lower() in {"nan", "none"}:
                df.at[idx, "error_label"] = ERROR_CODE_MAP.get(code_int, "unknown_evaluation_error")
...
# Find all XYZ structures, group them by molecule name, and organize paths by source.
def scan_structure_files() -> Dict[str, Dict[str, Path]]:
    return scan_structure_files_in_root(STRUCT_ROOT)
...
# Finds run folders in the combined_runs layout:
#
# combined_runs/
#   runs_closed/run_001, run_002, ...
#   runs_open/run_001, run_002, ...
#
# It also falls back to the old local runs/ layout if combined_runs/ is not present.
COMBINED_RUNS_DIR = BASE_DIR / "combined_runs"
RUN_GROUPS = ["runs_closed", "runs_open"]
GROUP_DISPLAY_NAMES = {
    "runs_closed": "Proprietary",
    "runs_open": "Open-Source",
    "runs": "Single-folder",
}

def model_type_label_from_group(group_name: str) -> str:
    return GROUP_DISPLAY_NAMES.get(str(group_name), str(group_name))

def discover_run_dirs() -> list[tuple[str, Path]]:
    discovered: list[tuple[str, int, Path]] = []

    if COMBINED_RUNS_DIR.exists():
        for group_name in RUN_GROUPS:
            group_dir = COMBINED_RUNS_DIR / group_name
            ...
            for p in group_dir.iterdir():
                ...
                if m:
                    discovered.append((group_name, int(m.group(1)), p))
        discovered.sort(key=lambda x: (x[0], x[1]))
        return [(group_name, p) for group_name, _, p in discovered]

    # Backward-compatible fallback: old layout BASE_DIR/runs/run_001, ...
    ...
    for p in runs_dir.iterdir():
        ...
        m = re.fullmatch(r"run_(\d{3})", p.name)
        if m:
            discovered.append(("runs", int(m.group(1)), p))
    discovered.sort(key=lambda x: x[1])
    return [(group_name, p) for group_name, _, p in discovered]

# Scans one run folder and groups PubChem plus every model subfolder by molecule name.
# This works for both closed-source runs (claude/gemini/openai) and open-source
# runs (deepseek/llama/mistral), and will also handle future model folders.
def discover_model_names_in_root(struct_root: Path) -> list[str]:
    if not struct_root.exists():
        return []
    names = []
    for p in struct_root.iterdir():
        if p.is_dir() and p.name != "pubchem":
            names.append(p.name)
    ordered = [m for m in MODEL_ORDER if m in names]
...
def scan_structure_files_in_root(struct_root: Path) -> Dict[str, Dict[str, Path]]:
    ...
    for model in discover_model_names_in_root(struct_root):
        model_dir = struct_root / model
        for path in model_dir.glob(f"*_{model}.xyz"):
            files[safe_stem_base(path, f"_{model}")][model] = path
...
def make_error_summary_outputs(df: pd.DataFrame, out_dir: Path) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    err_df = repair_error_columns(df.copy())
    ...
    error_pivot_reset["total"] = error_pivot_reset[MODEL_ORDER].sum(axis=1)
    ...
    present_models = [model for model in MODEL_ORDER if model in error_pivot.columns]
    width = 0.8 / max(1, len(present_models))
    fig, ax = plt.subplots(figsize=(max(10, len(labels) * 1.35), 6))
    for i, model in enumerate(present_models):
        vals = error_pivot[model].to_numpy(dtype=float)
        xpos = x + (i - (len(present_models) - 1) / 2) * width
    ...
    totals_by_error = error_pivot.sum(axis=1).to_numpy(dtype=float)
    y_min, y_max = ax.get_ylim()
    span = max(float(y_max - y_min), 1.0)
    max_total = float(np.max(totals_by_error)) if len(totals_by_error) else 0.0
    for xi, total in zip(x, totals_by_error):
        if total > 0:
            ax.text(
                xi,
                total + span * 0.04,
    ...
    ax.set_ylim(y_min, max(y_max, max_total + span * 0.16))
    fig.tight_layout()
...
def run_single_run(run_dir: Path, group_name: str = "") -> dict:
    run_name = run_dir.name
    full_run_name = f"{group_name}_{run_name}" if group_name else run_name
    model_type = model_type_label_from_group(group_name) if group_name else "Unknown"
    struct_root = run_dir / "structures"
    results_dir = run_dir / "results"
    aligned_root = results_dir / "aligned_xyz"
    out_dir = run_dir / "combined_rmsd_analysis"

    discovered_models = discover_model_names_in_root(struct_root)
    for model in discovered_models + ["pubchem_reference"]:
    ...
    rows = []
    print(f"\nStarting evaluation for {full_run_name} ...")
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for idx, future in enumerate(as_completed(future_to_task), start=1):
            ...
            rows.append(row)
            print(f"[{full_run_name} {idx}/{len(tasks)}] {row['model']} | {row['molecule']} | RMSD={row['rmsd']} | status={row['status']} | error={row.get('error_code')}: {row.get('error_label')}")

    df = pd.DataFrame(rows).sort_values(["molecule", "model"]).reset_index(drop=True)
    mapping_df = df[[
        "group", "model_type", "run", "molecule", "model", "atom_count", "status", "mapping_source", "permutation_ref_to_model",
        "pubchem_file", "model_file", "aligned_model_file", "note", "error_code", "error_label", "error_message"
    ]].copy()
...
def run_pipeline() -> dict:
    ...
    if not run_dirs:
        raise SystemExit("No run folders found under combined_runs/runs_closed, combined_runs/runs_open, or fallback runs/")
    ...
    for group_name, run_dir in run_dirs:
        result = run_single_run(run_dir, group_name=group_name)
        ...
        if not result["threshold_df"].empty:
            ...
            tdf["run"] = result["run"]
            tdf["group"] = group_name
            tdf["model_type"] = model_type_label_from_group(group_name)
    ...
    latest_group_name, latest_run_dir = run_dirs[-1]
    # Only sync to project root when using the old BASE_DIR/runs layout.
    # In combined_runs mode, each run keeps its own results folder, and the
    # grand-total script reads those directly.
    if latest_group_name == "runs":
        try:
    ...
    print(f"Latest processed run: {latest_group_name}/{latest_run_dir.name}")
